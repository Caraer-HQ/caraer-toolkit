...
4. Give this exact guidance:
   ...
   4. Enable all scopes, or only the specific scopes you want to grant access to.
...
6. If an authorization URL must be sent to the user's browser, send the exact URL generated by the OAuth client only through the private conversation. Never add, remove, reorder, or edit its `scope`, `state`, `code_challenge`, redirect URI, or other parameters.
